...
def neural_style_transfer(content_img_url: str, style_img_url: str, config: StyleTransferConfig):
    content_img_path = keras.utils.get_file(
        os.path.basename(content_img_url), content_img_url)
    style_img_path = keras.utils.get_file(
        os.path.basename(style_img_url), style_img_url)
    ...
    def compute_loss(combination_img, content_img, style_img):
        ...
        input_tensor = tf.concat(
            [content_img, style_img, combination_img], axis=0)
        ...
        loss += config.total_variation_weight * \
            total_variation_loss(combination_img)

        # Content loss
        layer_features = features[config.content_layer_name]
        content_features = layer_features[0, :, :, :]
        combination_features = layer_features[2, :, :, :]
        loss = loss + config.content_weight * \
    ...
    for i in range(1, config.num_iterations + 1):
        ...
        if i % config.save_interval == 0 or i == config.num_iterations:
            ...
            keras.preprocessing.image.save_img(
                f"{save_directory}/{file_name}", img)
